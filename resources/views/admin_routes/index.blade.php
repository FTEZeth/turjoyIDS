...
@section('content')
    ...
    <form class ="flex flex-col items-center w-1/2" action="#" method="POST" enctype="multipart/form-data">
        @csrf
        <div>
            <label class="block mb-2 text-sm font-medium text-gray-900 bg-red-500 dark:text-white" for="file_input"></label>
            ...
            @error('document')
                <p class="bg-red-400 font-semibold my-4 text-lg text-center text-red-800 px-4 py-3 rounded-lg text-center">
                    {{$message}}
                </p>
            @enderror
        </div>
